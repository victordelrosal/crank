...
Maintained by [Victor del Rosal](https://github.com/victordelrosal). Built and dogfooded with Claude Code on Opus 5, which is the gear it is tuned for; it works on any frontier-class model, and the weaker the model the more of the lifting the frame does.
...
- **DECOMPOSE** dispatches a fleet, one specialist per independent workstream. Every agent it sends out carries an explicit model choice picked by task shape, so you never stop to `/model`-switch mid-run: two gears, drive (`opus`) for everything that involves judgment, which is roughly 95% of a run, and low (`sonnet`) only for the genuinely mechanical passes. Judgment never downshifts, and a verifier is never below the builder it grades. The run opens by announcing which gear it is in and calls out every shift as it happens, because an automatic you cannot see is just a black box. Before dispatch it runs the fake-edge test on the plan (an edge only counts if the second agent consumes something the first produced) and audits for hidden edges, giving any two builders who must touch the same tree their own git worktrees. At fan-in it counts returns against dispatches, so a dead agent shows up as a named gap instead of a silence.
...
## The topology layer (harness, loop, graph)

**Three layers, named once, as a diagnostic and not a taxonomy to worship.** Harness is the environment: tools, permissions, routing, budget, logging. Loop is the feedback cycle: generate, act, evaluate, decide. Graph is the topology: what runs next, what fans out, what joins, where gates are. They nest: harness around graph, graph around loops. Crank is a loop skill whose DECOMPOSE step is its graph step; the harness is Claude Code's. Crank specifies what it needs from a harness, it does not build one. That is scope, not weakness.

**Aim the fix at the layer that owns the failure.** A stuck round often gets another round when the fault sat a layer away.

| Symptom | Layer |
|---|---|
| Cannot reach the data, lost its state, too much access | Harness |
| Close but unreliable, spins forever, declares done without proof | Loop |
| Specialists in the wrong order, cannot tell which stage broke | Graph |
| Work waiting on nothing | Graph: the edge is fake |
| The topology changes every run | Neither: stop formalizing, simplify the harness |

**The fan-in guard comes first.** In a chain a dead step stops everything and you notice. In a fan-out, one dead agent among twenty is a gap in a report that looks complete. So every fan-in asserts its count: returns against dispatches, and any difference gets named in `LOG.md` and the handoff. `.filter(Boolean)` without a count assertion is Crank's honesty doctrine leaking out through a convenience method.

**Anchors are the second.** A fleet where every node's evidence is another node's report is one loop in a costume: internally consistent, entirely unverified, and more expensive. Every criterion must end on something that cannot argue back: a test that ran, a URL that returned 200, a file at the path, a number from the environment. Adding verifiers adds confidence, not truth. With no anchor, a criterion is marked judge-checkable, not fact.

**The rest lands at DECOMPOSE.** Each ordered pair gets the fake-edge test: does B consume an artifact A produced? If not, the edge is imaginary and the wait is pure latency. Every agent gets a node contract: a bounded job, a defined input, a defined output shape, declared before dispatch. Free prose is a node only a human can read. Hidden edges are shared *resources*, not shared data: two briefs that never mention each other are still coupled if they write the same file or branch, so each gets its own worktree or a real edge. Fan-in reduces in code, not tokens, and layers: summarize each batch, then combine. Formalize topology only once traces show the real edges, and reach for Workflow only when the topology is real: many items, multiple stages, per-item verification, a code-enforced ceiling, crash resume.

**Where this came from.** Two X articles, late July 2026, both captured in [`research/`](research/): beamnxw on the three layers (2026-07-25) and Anatoli Kopadze on graph mechanics (2026-07-24). The taxonomy article is vendor-shaped and its closing section reads as unedited generation, so it is cited for the lens, not as authority. Kopadze's heavy-fan-out figures (the Bun rewrite, roughly $165,000) are reported, not verified here. Refused: a third "graph mode", since DECOMPOSE already is one, and dozens of nodes per run as a default.

**One word, two disciplines.** A rival July 2026 usage gives "graph engineering" to the shared *knowledge graph*: the entity-and-relation memory agents query across sessions, where every edge carries provenance. That is a memory discipline, not an orchestration one, and the two are not parent and child; they share the mathematics of nodes and edges the way a call graph and a social graph share it, and nothing else. Crank does not use the word that way. Here "graph" is always the topology of one run, and the memory sense is always written "knowledge graph" in full. Crank's own cross-run memory is prose rules in `LEARNINGS.md` plus `LOG.md` and `STATE.json`; calling that a knowledge graph would overstate it. Weighed and mostly refused on 2026-07-27, with the argument and the source's four sourcing defects in [`research/knowledge-graph-playbook-anthropic-2026-07.md`](research/knowledge-graph-playbook-anthropic-2026-07.md). Worth knowing: the layered stack both usages inherit is a third-party framing, not Anthropic's. A direct search of Anthropic's engineering writing that day found no four- or five-layer stack of any kind.
